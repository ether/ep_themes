# Contributing to ep_themes

Thanks for helping improve `ep_themes` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_themes` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_themes from your local clone
pnpm run plugins i --path ../ep_themes

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_themes/` are picked up after a server restart.
...
* Etherpad version, ep_themes version, browser + OS
* Steps to reproduce
